...
squat.rep <- generalstrength %>%
  ...
  mutate(squat_reps = 5,
         perc_1rm = squat_weight/adj_perc_1RM_RIR(squat_reps, max_perc_1RM_func = max_perc_1RM_linear, adjustment = 1))

dl.rep <- generalstrength %>%
  filter(deadlift_sets_completed == 1) %>%
  select(date, date_index, starts_with("deadlift")) %>%
  mutate(deadlift_reps = 5,
         perc_1rm = deadlift_weight/adj_perc_1RM_RIR(deadlift_reps, max_perc_1RM_func = max_perc_1RM_linear, adjustment = 1))
